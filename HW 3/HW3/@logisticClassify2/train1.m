...
%     z = obj.wts*X1(i,:)';
%     sigma = 1/(1+exp(-z));
%     Jsur(iter) = Jsur(iter)-Y(i,:)*log(sigma)-(1-Y(i,:))*(log(1-sigma))+ reg*(sum((obj.wts).^2));   %%% TODO: compute surrogate (neg log likelihood) loss
%   end 
  ...
  for j=1:n,
    ...
    n
    iter
    J_grad = -Y(j,:)*sigma*exp(-z)*X1(j,:)+(1-Y(j,:))*sigma*X1(j,:)+ 2*reg*obj.wts   % The last term doesn't seem right
    obj.wts = obj.wts - step * J_grad;      % take a step down the gradient
  end;
